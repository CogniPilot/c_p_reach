...
def run():
    ...
    parser = argparse.ArgumentParser(f'main {version:s}')
    ...
    parser.add_argument("--multirotor", action='store_true')
    parser.add_argument("--position_target", default=2.0)
    parser.add_argument("--attitude_target", default=0.5)
    parser.add_argument("--thrust_dist", default=0.0)
    parser.add_argument("--gyro_noise", default=0.01)# rad/s gyro disturbance bound.
    parser.add_argument("--plots", default='fig2')
    args = parser.parse_args()


    # Determine C_P_Reach run type.
    do_multirotor = args.multirotor
    do_rover = args.rover
    if args.plots:
    ...
    # Mulirotor Analysis
        ...
        w1 = args.thrust_dist # disturbance for translational (impact a)  thrust disturbance for outer loop
        ...
        ref = mr_plan.traj_from_coords(args.trajectory)
        ...
        if do_plots:
            ...
            plt.axis('auto')
            plt.tight_layout()
            plt.savefig(f'{args.plots}/reference_trajectory.png')
            plt.close()

        # Compute Reachable set for Rotation
       ...
       print("Please specify exploit type. Options are '--rover', and '--multirotor'") 
